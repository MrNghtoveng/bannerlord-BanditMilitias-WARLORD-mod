...
namespace BanditMilitias.Intelligence.Strategic
{
    ...
    public class WarlordSystem : BanditMilitias.Core.Components.MilitiaModuleBase
    {
        ...
        private void DetermineInitialProperty(Warlord warlord, Hero hero)
        {
            ...
            var nearbySettlements = Settlement.All
                .Where(s => CompatibilityLayer.GetSettlementPosition(s).Distance(CompatibilityLayer.GetHeroPosition(hero).AsVec2) < 100f)
                .OrderBy(s => CompatibilityLayer.GetSettlementPosition(s).Distance(CompatibilityLayer.GetHeroPosition(hero).AsVec2))
                .ToList();
            ...
            lock (_initLock)
            {
                ...
                try
                {
                    ...
                    if (fallenEvt != null)
                    {
                        ...
                        EventBus.Instance.Publish(fallenEvt);
        ...
        [Obsolete("Warlords now emerge organically via TryPromoteCaptainToWarlord")]
        private void SeedInitialWarlords() { }
        ...
        public bool TryPromoteCaptainToWarlord(MobileParty militia)
        {
            ...
            // Warlord terfisi iÃ§in ana eÅik: 500 puan + min asker (Nam Ã§ok yÃ¼ksekse asker sÄ±nÄ±rÄ± esner)
            ...
            InformationManager.DisplayMessage(new InformationMessage(
                $"?? {promotionCaptain.Name} ({score:F0} Nam) {promotionHideout.Name} konumunda bir SAVAÅ LORDU olarak yÃ¼kseldi!",
                Colors.Magenta));
        ...
        private void CheckAndApplyChaosNerf()
        {
            ...
            var activeWarlords = GetAllWarlords().Where(w => w.IsAlive).ToList();
