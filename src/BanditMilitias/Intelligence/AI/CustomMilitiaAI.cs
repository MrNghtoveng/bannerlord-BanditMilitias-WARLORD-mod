    ...
    public static class CustomMilitiaAI
    {
        ...
        private static MobileParty? FindBestRaidTarget(MobileParty me)
        {
            ...
            foreach (var p in nearby)
            {
                ...
                if (isMilitia)
                {
                    ...
                    if (warlord != null && warlord.Gold < 5000)
                    {
                        predatoryScore *= 2.5f; // Economic desperation makes them elite hunters
                    }
        ...
        private static bool TryRetreatFromOverwhelmingThreat(MobileParty party, MilitiaPartyComponent component)
        {
            ...
            foreach (MobileParty candidate in nearby)
            {
                ...
                if (DecisionRules.IsOverwhelmingThreat(myStrength, enemyStrength, overwhelmRatio) &&
            ...
            else
            {
                ...
                if (!fleeDir.IsValid)
                {
                    fleeDir = new Vec2(1f, 0f);
                }
                Vec2 fleePos = myPos + (fleeDir * 30f);
                SetMoveGoTo(party, fleePos);
            }
