    ...
    public class FearSystem : MilitiaModuleBase
    {
        ...
        private void TriggerBetrayal(string settlementId, SettlementFearState state)
        {
            ...
            if (!string.IsNullOrEmpty(state.ControllingWarlordId))
            {
                ...
                EventBus.Instance.Publish(threatEvt);
                ...
                if (warlord != null)
                {
                    ...
                    EventBus.Instance.Publish(betrayalEvt);
        ...
        private void UpdateDiagnosticSnapshot()
        {
            ...
            try
            {
                float avgFear = _fearStates.Values.Average(s => s.Fear);
                float avgRespect = _fearStates.Values.Average(s => s.Respect);
                int highFear = _fearStates.Values.Count(s => s.Fear > 0.70f);
                int resisting = _fearStates.Values.Count(s => s.IsResisting);
                int controlled = _fearStates.Values.Count(s => !string.IsNullOrEmpty(s.ControllingWarlordId));

                _cachedDiagnosticSnapshot = $"Tracked: {_fearStates.Count} | Controlled: {controlled}\n" +
