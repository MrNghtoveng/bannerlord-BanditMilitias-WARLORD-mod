...
namespace BanditMilitias.Systems.Tracking
{
    ...
    public sealed class PlayerTracker : BanditMilitias.Core.Components.MilitiaModuleBase, IPlayerTracker
    {
            ...
            try
            {
                ...
                if (Settings.Instance?.TestingMode == true)
                {
                    DebugLogger.Info("PlayerTracker",
                        $"[Threat Change] {oldThreat:F2} Â› {newThreat:F2} ({reason})");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (Hero.MainHero == null) return 0f;
        ...
        {
            return "PlayerTracker (FIXED):\n" +
                   $"  Threat Level: {GetThreatLevel():F2}\n" +
                   ...
                   $"  Heatmap Cells: {_routeHeatmap.Count} (pruned {_heatmapPrunes}Ã—)\n" +
