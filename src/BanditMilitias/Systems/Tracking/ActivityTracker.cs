...
namespace BanditMilitias.Systems.Tracking
{
    ...
    public class CaravanActivityTracker : BanditMilitias.Core.Components.MilitiaModuleBase
    {
        ...
        private void ApplyDecay()
        {
            ...
            foreach (var kvp in _tradeIntensityGrid.ToList())
            {
                float newIntensity = kvp.Value * (1.0f - DECAY_RATE);
                if (newIntensity < 0.1f)
                    keysToRemove.Add(kvp.Key);
                else
                    _tradeIntensityGrid[kvp.Key] = newIntensity;
            }
        ...
        private void CleanupOldEvents()
        {
            ...
            var validEvents = _recentEvents.Where(e => e.Timestamp > cutoffTime).ToList();
            _recentEvents = new Queue<TradeEvent>(validEvents);
        }
    ...
    public class WarActivityTracker : BanditMilitias.Core.Components.MilitiaModuleBase
    {
        ...
        private void ApplyIntensityDecay()
        {
            ...
            foreach (var kvp in _warIntensityGrid.ToList())
            {
                float newIntensity = kvp.Value * (1.0f - INTENSITY_DECAY_RATE);
                ...
                {
                    _warIntensityGrid[kvp.Key] = newIntensity;
                }
            }
        ...
        public void CleanupOldEvents()
        {
            ...
            _recentEvents = new Queue<WarEvent>(validEvents);
        }
