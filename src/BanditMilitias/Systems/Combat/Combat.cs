...
namespace BanditMilitias.Systems.Combat
{
    ...
    public class WarlordCombatSystem : MissionBehavior
    {
            ...
            in MissionWeapon affectorWeapon, in Blow blow, in AttackCollisionData collisionData)
        {
            ...
            if (affectedAgent.Character.IsHero) return; // sadece AI militias
            ...
            // Hafif rejenerasyon
            if (affectedAgent.Health < affectedAgent.HealthLimit * 0.85f)
                affectedAgent.Health = System.Math.Min(
                    affectedAgent.HealthLimit,
                    affectedAgent.Health + 0.5f);
        }
    ...
    public class WarlordRegenerationSystem : MissionBehavior
    {
        ...
        public override void OnMissionTick(float dt)
        {
            ...
            for (int i = _activeWarlords.Count - 1; i >= 0; i--)
            {
                ...
                if (data.Agent.Health < data.Agent.HealthLimit)
                {
                    _ = data.Agent.Health;
                    data.Agent.Health += data.HealAmountPerTick;

                    if (data.Agent.Health > data.Agent.HealthLimit)
                        data.Agent.Health = data.Agent.HealthLimit;

                    if (data.HealAmountPerTick > 2.0f && Settings.Instance?.ShowTestMessages == true)
                    {
            ...
            try
            {
                ...
                if (winner.PartyComponent is MilitiaPartyComponent winnerComp)
                {
                    ...
                    try
                    {
                        ...
                        Core.Events.EventBus.Instance.Publish(battleEvt);
                ...
                if (mergedMilitias.Count > 0)
                {
                    ...
                    Core.Events.EventBus.Instance.Publish(mergeEvt);
                ...
                if (MBRandom.RandomFloat < 0.2f && winner.MemberRoster != null)
                {
                    var eliteTroops = winner.MemberRoster.GetTroopRoster()
                        .Where(t => t.Character != null && t.Character.Tier >= 3)
                        .ToList();

                    if (eliteTroops.Any())
                    {

                        var troopToUpgrade = eliteTroops[MBRandom.RandomInt(eliteTroops.Count)];
