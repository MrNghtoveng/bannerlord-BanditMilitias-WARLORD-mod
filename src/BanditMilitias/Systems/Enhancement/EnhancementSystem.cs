...
namespace BanditMilitias.Systems.Enhancement
{
    ...
    public class BanditEnhancementSystem : BanditMilitias.Core.Components.MilitiaModuleBase
    {
        ...
        public override bool IsEnabled => Settings.Instance?.EnhancedBandits ?? false;
    ...
    public class HeroicFeatsSystem : BanditMilitias.Core.Components.MilitiaModuleBase
    {
        ...
        public override bool IsEnabled => Settings.Instance?.EnableHeroicFeats ?? false;
        ...
        {
            _cachedRewardItems ??= new System.Collections.Generic.List<TaleWorlds.Core.ItemObject>();
        }
        ...
        public void TryAwardAttributePoint(Hero hero, float powerRatio)
        {
            ...
            // Standart level-up gating dışında maksimum 5 ekstra attribute izni.
        ...
        public void TryAwardFocusPoint(Hero hero, float powerRatio)
        {
            ...
            // FIX S3: Focus point birikimi için yumuşak tavan.
        ...
        {
            if (_cachedRewardItems == null)
            {
                _cachedRewardItems = new();
                foreach (var i in TaleWorlds.ObjectSystem.MBObjectManager.Instance.GetObjectTypeList<ItemObject>())
                {
                    if ((i.HasWeaponComponent || i.HasArmorComponent || i.HasHorseComponent || i.HasSaddleComponent) &&
                        !i.NotMerchandise && i.Value > 1000)
                    {
                        _cachedRewardItems.Add(i);
                    }
                }
            }

            if (_cachedRewardItems.Count == 0) return null;

            int minValue = (int)(powerRatio * 2000);

            List<ItemObject> validItems = new();
            foreach (var i in _cachedRewardItems)
            {
                if (i.Value >= minValue) validItems.Add(i);
            }

            if (validItems.Count == 0) validItems = _cachedRewardItems;

            return validItems[MBRandom.RandomInt(validItems.Count)];
        }

        public void AwardWarlordVictoryRewards(Hero playerHero, LegitimacyLevel defeatedLevel)
        {
            if (playerHero == null) return;
            if (!IsEnabled) return;
            if (defeatedLevel < LegitimacyLevel.Rebel) return;
            ...
            switch (defeatedLevel)
            {
                case LegitimacyLevel.Rebel:
                    {
                        ...
                        GiveGoldReward(gold);
                        InformationManager.DisplayMessage(new InformationMessage(
                            $"?? İSYANCI LİDERİ YOK EDİLDİ! +{gold} dinar ödül!", Colors.Green));
                    }
                    break;

                case LegitimacyLevel.Warlord:
                    {
                        // Combined logic for Warlord death/defeat
                        int gold = (int)(defeatedLevel == LegitimacyLevel.Warlord ? 5000 * rewardMultiplier : 15000 * rewardMultiplier);
                        float renown = (defeatedLevel == LegitimacyLevel.Warlord ? 50f : 150f) * rewardMultiplier;
                        
                        GiveGoldReward(gold);
                        GiveRenownReward(renown);
                        TryAwardHeroItem(playerHero, defeatedLevel == LegitimacyLevel.Warlord ? 3.0f : 8.0f);

                        if (defeatedLevel == LegitimacyLevel.Recognized)
                        {
                            TryAwardAttributePoint(playerHero, 12.0f);
                            TryAwardFocusPoint(playerHero, 8.0f);
                        }

                        string titleTier = defeatedLevel == LegitimacyLevel.Warlord ? "SAVAŞ LORDU YENİLDİ" : "HÜKÜMDAR DÜŞTÜ";
                        InformationManager.DisplayMessage(new InformationMessage(
                            $"[{titleTier}] +{gold} dinar, +{renown:F0} ün ve ganimet!", 
                            defeatedLevel == LegitimacyLevel.Warlord ? Colors.Yellow : Colors.Magenta));
                ...
                case LegitimacyLevel.Recognized:
                    {
                        int gold = (int)(15000 * rewardMultiplier);
                        float renown = 150f * rewardMultiplier;
                        GiveGoldReward(gold);
                        GiveRenownReward(renown);
                        TryAwardHeroItem(playerHero, 8.0f);
                        ...
                        InformationManager.DisplayMessage(new InformationMessage(
                            $"[{titleTier}] +{gold} dinar, +{renown:F0} ün ve EFSANEVİ ganimet!",
                            Colors.Magenta));
        ...
        private void GiveGoldReward(int gold)
        {
            ...
            catch (System.Exception ex) { BanditMilitias.Debug.DebugLogger.Log($"[HeroicFeats] Gold API failed: {ex.Message}"); }
        }
        ...
        private void GiveRenownReward(float renown)
        {
            try
            {
                var mainClan = Hero.MainHero?.Clan;
                if (mainClan != null)
                    mainClan.AddRenown(renown);
            }
            catch (System.Exception ex) { BanditMilitias.Debug.DebugLogger.Log($"[HeroicFeats] Clan API failed: {ex.Message}"); }
        }

        public void ShowUnderdogMessage(float powerRatio, int goldBonus, float renownBonus)
        {
            if (powerRatio < 1.5f || Settings.Instance?.EnableHeroicFeats != true) return;

            string tier = powerRatio >= 10.0f ? "GOODLÝKE" :
                          powerRatio >= 5.0f ? "LEGENDARY" :
                          powerRatio >= 3.0f ? "EPIC" :
                          powerRatio >= 2.0f ? "HEROIC" : "BRAVE";

            Color color = powerRatio >= 5.0f ? Colors.Magenta : Colors.Green;

            var message = $"{tier} VICTORY! Power Ratio: {powerRatio:F1}x | Bonus: +{goldBonus} Gold, +{renownBonus:F1} Renown";
            InformationManager.DisplayMessage(new InformationMessage(message, color));
        }
#if DEBUG

        [CommandLineFunctionality.CommandLineArgumentFunction("sys_gl_override", "bandit")]
        public static string TestGodlikeVictory(System.Collections.Generic.List<string> args)
        {
            if (Hero.MainHero == null) return "Main Hero not found.";

            InformationManager.DisplayMessage(new InformationMessage("? SYSTEM OVERRIDE: VICTORY SIMULATION...", Colors.Red));

            float fakePowerRatio = 20.0f;
            HeroicFeatsSystem.Instance.ShowUnderdogMessage(fakePowerRatio, 10000, 100f);
            HeroicFeatsSystem.Instance.TryAwardAttributePoint(Hero.MainHero, fakePowerRatio);
            HeroicFeatsSystem.Instance.TryAwardFocusPoint(Hero.MainHero, fakePowerRatio);
            HeroicFeatsSystem.Instance.TryAwardHeroItem(Hero.MainHero, fakePowerRatio);

            return "Override complete.";
        }

        [CommandLineFunctionality.CommandLineArgumentFunction("sys_item_debug", "bandit")]
        public static string TestItemReward(System.Collections.Generic.List<string> args)
        {
            if (Hero.MainHero == null) return "Main Hero not found.";
            InformationManager.DisplayMessage(new InformationMessage("? CHEAT: Generating Legendary Item...", Colors.Magenta));
            HeroicFeatsSystem.Instance.TryAwardHeroItem(Hero.MainHero, 15.0f);
