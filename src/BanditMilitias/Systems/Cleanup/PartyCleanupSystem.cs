    ...
    public class PartyCleanupSystem : MilitiaModuleBase, ICleanupSystem
    {
        ...
        public void OnPartyDestroyedCleanup(MobileParty party, PartyBase partyBase)
        {
            if (party?.PartyComponent is MilitiaPartyComponent comp)
            {
                ...
                BanditMilitias.Core.Events.EventBus.Instance.Publish(disbandEvt);
        ...
        private void PerformUninstallCleanup()
        {
            ...
            int deletedCount = 0;
            var toDelete = new List<MobileParty>();
        ...
        private void PerformDeepClean()
        {
            ...
            var deadParties = _gracePeriodTracker.Keys.Where(p => !p.IsActive && p.MemberRoster?.TotalManCount <= 0).ToList();
            foreach (var dead in deadParties) _ = _gracePeriodTracker.Remove(dead);
        }
        ...
        private void LogCleanupAction(string partyName, string reason)
        {
            try
            {
                string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
                    "Mount and Blade II Bannerlord", "AIData");
        ...
        /// </summary>
        private void TransferToNearestCaptain(MobileParty headlessParty)
        {
            ...
            if (target == null)
            {
                // Hiç Captain yoksa direkt yok et
                DebugLogger.Info("CleanupSystem",
                    $"[Headless] No captain found for {headlessParty.Name}, destroying directly.");
                ExecuteNuclearCleanup(headlessParty);
                return;
            }
            ...
            // Transfer sonrası headless partiyi yok et
            ExecuteNuclearCleanup(headlessParty);
        }
