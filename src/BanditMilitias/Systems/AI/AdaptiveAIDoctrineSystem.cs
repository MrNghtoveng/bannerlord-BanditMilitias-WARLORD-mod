...
namespace BanditMilitias.Systems.AI
{
    ...
    public class AdaptiveAIDoctrineSystem : MilitiaModuleBase
    {
        ...
        private void RecomputeDoctrineProfiles()
        {
            ...
            var playerArmy = Systems.Enhancement.WarlordTacticsSystem.Instance.AnalyzePlayerArmy();
            ...
            List<Warlord> warlords = WarlordSystem.Instance.GetAllWarlords()
            ...
            BanditMilitias.Systems.Progression.LegitimacyLevel warlordLevel)
        {
            ...
            bool collectData = Settings.Instance?.EnableMLDataCollection ?? true;
            int sampleEvery = Math.Max(1, Settings.Instance?.MLLogEveryNBattles ?? 1);
            if (collectData && (_observedDoctrineSamples % sampleEvery == 0))
            {
                AdaptiveDoctrineDataLogger.LogProfileUpdate(
                    profile.WarlordId,
                    isGlobalProfile,
                    observed,
                    oldDoctrine,
                    ...
                    switched,
                    profile.Confidence,
                    profile.AggressionBias,
                    threatLevel,
                    style,
                    personality,
                    _observedDoctrineSamples);
                _loggedDoctrineSamples++;
            }
        }
        ...
        private void OnMapEventEnded(MapEvent mapEvent)
        {
            ...
            lock (_stateLock)
            {
                ...
                foreach (var involved in mapEvent.InvolvedParties)
                {
                    ...
                    bool collectData = Settings.Instance?.EnableMLDataCollection ?? true;
                    int sampleEvery = Math.Max(1, Settings.Instance?.MLLogEveryNBattles ?? 1);
                    if (collectData && (_observedBattleSamples % sampleEvery == 0))
                    {
                        AdaptiveDoctrineDataLogger.LogBattleUpdate(
                            profile.WarlordId,
                            party.StringId,
                            won,
                            confidenceBefore,
    ...
    public static class AdaptiveDoctrineRules
    {
        ...
        public static float GetChaseDistanceMultiplier(CounterDoctrine doctrine, bool isRaider)
        {
            return doctrine switch
            {
                CounterDoctrine.FastFlank => isRaider ? 1.35f : 1.20f,
                CounterDoctrine.ShockRaid => isRaider ? 1.30f : 1.10f,
                CounterDoctrine.DefensiveDepth => 0.75f,
                CounterDoctrine.SpearWall => 0.90f,
                _ => 1.00f
            };
        }
        ...
        public static bool ShouldSwitchDoctrine(
            ...
            float confidence,
            float hoursSinceLastSwitch,
            float cooldownHours)
        {
            if (current == candidate)
                return false;

            if (hoursSinceLastSwitch < Math.Max(1f, cooldownHours))
                return false;

            float clampedConfidence = Clamp01(confidence);

            if (candidate == CounterDoctrine.DefensiveDepth)
                return true;

            if (clampedConfidence <= 0.35f)
                return true;

            // BUG-6 KALICI ÇÖZÜM: Yüksek Confidence nedeniyle Warlord'un SpearWall
            // veya başka spesifik bir doktrinde takılı kalmasını (Deadlock) engeller.
            if (current != CounterDoctrine.Balanced && candidate != current)
            {
                // Eğer son rotasyondan beri 12 saat geçmişse VEYA özgüven %90'ın altındaysa hemen değiştir:
                if (hoursSinceLastSwitch > 12f || clampedConfidence < 0.90f)
                    return true;
            }

            return clampedConfidence <= 0.60f && candidate != CounterDoctrine.Balanced;
        }

        public static float UpdateConfidence(float currentConfidence, bool success, float learningRate)
        {
            float rate = Clamp01(learningRate);
            // Reward for success is slightly higher now to allow climbing out of the 0.05 floor
            float delta = success ? (0.22f * rate) : (-0.15f * rate);
            return Clamp(currentConfidence + delta, 0.05f, 0.99f);
        }

        private static CounterDoctrine ResolveMixedArmsCounter(PlayStyle playStyle, PersonalityType personality)
        {
            if (playStyle == PlayStyle.Aggressive || personality == PersonalityType.Aggressive)
                return CounterDoctrine.ShockRaid;

            if (playStyle == PlayStyle.Defensive || personality == PersonalityType.Cautious)
                return CounterDoctrine.DefensiveDepth;

            if (personality == PersonalityType.Cunning)
                return CounterDoctrine.FastFlank;
