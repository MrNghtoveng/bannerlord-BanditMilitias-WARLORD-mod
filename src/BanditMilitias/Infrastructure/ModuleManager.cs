    ...
    public class ModuleManager
    {
                ...
                try
                {
                    ...
                    if (Settings.Instance?.TestingMode == true)
                    {
                        DebugLogger.Info("ModuleManager",
                            ...
                            $"({enabledModules.Count} modules, {_failedModules.Count} failed)");
                    }
                }
                catch (Exception ex)
                {
        ...
        private void ProcessModuleTicks(Action<IMilitiaModule> tickAction, string tickType)
        {
            ...
            foreach (var module in moduleSnapshot)
            {
                ...
                string moduleName = ResolveModuleName(module);
                string scopeName = $"Tick.{tickType}.{moduleName}";

                if (!module.IsCritical && IsModuleFailed(moduleName)) continue;

                bool trackPerf = Settings.Instance?.TestingMode == true;
                var perfWatch = trackPerf ? System.Diagnostics.Stopwatch.StartNew() : null;
                ...
                finally
                {
                    ...
                    {
                        perfWatch.Stop();
                        RecordModuleTick(moduleName, perfWatch.Elapsed.TotalMilliseconds);
                    }
        ...
        public string GetDiagnostics()
        {
            lock (_initLock)
            {
                int enabledModules = _modules.Count(m => m.IsEnabled);
