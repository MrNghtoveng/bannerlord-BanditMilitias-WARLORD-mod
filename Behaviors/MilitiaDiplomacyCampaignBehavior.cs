...
namespace BanditMilitias.Behaviors
{
                    ...
                    if (mapEvent.InvolvedParties.Contains(captor))
                    {
                        try
                        {
                            ...
                            InformationManager.DisplayMessage(new InformationMessage(
                                new TextObject("{=BM_Escape_Battle}You managed to escape during the confusion of the battle!").ToString(),
                                Colors.Green));
                        }
                        catch (Exception ex)
                        {
        ...
        private void AddDialogs(CampaignGameStarter starter)
        {
            ...
            _ = starter.AddDialogLine("militia_start_hostile", "start", "militia_intro",
                "{=BM_Diplo_Start_Dynamic}{DYNAMIC_INTRO}",
                () =>
                {
                    if (!IsMilitiaParty() || MobileParty.ConversationParty == null || MobileParty.ConversationParty.MapFaction == null || Hero.MainHero?.MapFaction == null || !MobileParty.ConversationParty.MapFaction.IsAtWarWith(Hero.MainHero.MapFaction))
                        return false;

                    MBTextManager.SetTextVariable("DYNAMIC_INTRO", GetDynamicIntro(MobileParty.ConversationParty));
                    return true;
                },
                null);
            ...
            _ = starter.AddDialogLine("militia_duel_accept", "militia_duel_result", "close_window",
               ...
               () =>
               {
                   ...
                   BanditMilitias.Systems.Diplomacy.DuelSystem.StartDuel(MobileParty.ConversationParty);
        ...
        private void RecruitParty()
        {
            ...
            if (freeSpace <= 0)
            {
                InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=BM_Recruit_Fail_Space}Not enough space in party.").ToString(), Colors.Red));
                return;
            }
            ...
            var troops = new System.Collections.Generic.List<TroopRosterElement>();
            ...
            troops = troops.OrderBy(t => t.Character.Level).ToList();
            ...
            if (transferred <= 0)
            {
                InformationManager.DisplayMessage(new InformationMessage(new TextObject("{=BM_Recruit_Fail_Generic}Recruitment attempt failed.").ToString(), Colors.Red));
                return;
            }
            ...
            TextObject successMsg = new TextObject("{=BM_Recruit_Success_Msg}{COUNT} militia joined your party.");
            _ = successMsg.SetTextVariable("COUNT", transferred);
            InformationManager.DisplayMessage(new InformationMessage(successMsg.ToString(), Colors.Green));
        }
        private TextObject GetDynamicIntro(MobileParty party)
        {
            var warlord = WarlordSystem.Instance.GetWarlordForParty(party);
            if (warlord == null)
                return new TextObject("{=BM_Diplo_Start}Halt! This is Warlord territory. Pay the toll or bleed.");
