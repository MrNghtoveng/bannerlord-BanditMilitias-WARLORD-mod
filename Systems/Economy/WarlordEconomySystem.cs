    ...
    public class WarlordEconomySystem : MilitiaModuleBase
    {
        ...
        public override void OnDailyTick()
        {
            ...
            foreach (var warlord in allWarlords)
            {
                ...
                SellLootAtVillage(warlord);

                TryPurchaseEquipmentUpgrades(warlord);
                UpdateAnimalProgression(warlord);

                UpdateInventoryProgression(warlord);

                SyncPartyTradeGold(warlord);
                ...
                if (neededHorses > 5)
                {
                    ...
                    if (w.Gold >= cost)
                    {
                        ...
                        if (Settings.Instance?.TestingMode == true)
                            DebugLogger.TestLog($"[ECONOMY] {w.Name} purchased {neededHorses} horses for party: -{cost} Gold.");
                    }
                }
        ...
        private void UpdateInventoryProgression(Warlord w)
        {
            foreach (var party in w.CommandedMilitias)
            {
                ...
                if (totalLootValue > 2000)
                {
                    ...
                    float boost = totalLootValue / 100000f;
            ...
            foreach (var party in w.CommandedMilitias)
            {
                ...
                if (w.InfluencedVillages.Contains(party.CurrentSettlement))
                {
                    ...
                    for (int i = 0; i < party.ItemRoster.Count; i++)
                    {
                        ...
                        {
                            lootValue += (int)(item.Amount * item.EquipmentElement.Item.Value * 0.4f);
                    ...
                    if (lootValue > 0)
                    {
                        ...
                        if (Settings.Instance?.TestingMode == true)
                            DebugLogger.TestLog($"[ECONOMY] {w.Name} sold loot in village (Shadow Trade): +{lootValue} Gold.");
                    }
                }
            }
        }
            ...
            foreach (var party in w.CommandedMilitias)
            {
...

                {
                    ...
                    for (int i = 0; i < party.PrisonRoster.Count; i++)
                    {
                        ...
                        prisonerValue += element.Number * (element.Character.Tier * 15 + 20);
                    ...
                    if (Settings.Instance?.TestingMode == true)
                        DebugLogger.TestLog($"[ECONOMY] {w.Name} sold prisoners at hideout: +{prisonerValue} Gold.");
                }
            }
        }
        ...
        private void SyncPartyTradeGold(Warlord w)
        {
            ...
            foreach (var party in w.CommandedMilitias)
            {
                ...
                if (party.PartyTradeGold < targetGold)
                {
                    ...
                    int actualTransfer = (int)Math.Min(needed, w.Gold * 0.1f);
        ...
        {
            var tier = WarlordCareerSystem.Instance.GetTier(w.StringId);
            if (tier > CareerTier.FamousBandit && w.Gold < 30000) return;
            ...
            if (w.Gold >= goal)
            {
                ...
                if (Settings.Instance?.TestingMode == true)
                    DebugLogger.Info("Economy", $"[UPGRADE] {w.Name} reached {goal} gold target. Equipment upgraded. Spent: {spend:F0}");
            }
        }
        ...
        private void UpdateInfluencedVillages(Warlord w)
        {
            ...
            {
                if (Infrastructure.CompatibilityLayer.GetSettlementPosition(s).DistanceSquared(hideoutPos) < 625f)
        ...
        {
            CareerTier.Outlaw or CareerTier.Rebel => EconomicStage.Survival,
            CareerTier.FamousBandit => EconomicStage.Predatory,
            CareerTier.Warlord => EconomicStage.Feudal,
            CareerTier.Recognized => EconomicStage.Sovereign,
            CareerTier.Conqueror => EconomicStage.Royal,
        ...
        private float CalculateIncome(Warlord w, EconomicStage stage)
        {
            ...
            {
                EconomicStage.Survival => 0.8f,

                EconomicStage.Predatory => 1.2f,

                EconomicStage.Feudal => 1.5f,

                EconomicStage.Sovereign => 3.5f,

                EconomicStage.Royal => 5.0f,

                _ => 1f
            };


            float legitimacy = WarlordLegitimacySystem.Instance.GetPoints(w.StringId);
            ...
            if (globalAvg > 0)
                prosperityMulti = 0.8f + (regionalProsperity / globalAvg * 0.4f);
        ...
        private float CalculateExpenses(Warlord w, EconomicStage stage)
        {
            ...
            float corruption = 0f;
            if (w.Gold > 50000) corruption = (w.Gold - 50000) * 0.05f;
            ...
            float bureaucracyCost = territoryCount * 150f;
        ...
        public void ProcessMerging(Warlord w)
        {
            ...
            for (int i = 0; i < smallParties.Count - 1; i += 2)
            {
                ...
                if (pos1.DistanceSquared(pos2) < 100f)
        ...
        private void MergeParties(MilitiaPartyComponent p1, MilitiaPartyComponent p2)
        {
            ...
            NeuralEventRouter.Instance.Publish(new WarlordFallenEvent { Warlord = null });
