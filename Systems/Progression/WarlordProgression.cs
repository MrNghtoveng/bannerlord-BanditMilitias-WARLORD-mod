...
namespace BanditMilitias.Systems.Progression
{
    ...
    public class WarlordCareerSystem : MilitiaModuleBase
    {
        ...
        private void ApplyPromotion(Warlord w, CareerRecord rec, CareerTier tier)
        {
            ...
            switch (tier)
            {
                ...
                case CareerTier.Warlord:
                    ...
                    try 
                    {
                        BanditMilitias.Systems.Workshop.WarlordWorkshopSystem.Instance?.AddWorkshop(w.StringId, BanditMilitias.Systems.Workshop.WorkshopType.WeaponSmith);
                    }
                    catch (Exception ex) { DebugLogger.Warning("WarlordCareer", $"Failed to add weaponsmith workshop: {ex.Message}"); }
                ...
                case CareerTier.Taninmis:
                    ...
                    try 
                    {
                        BanditMilitias.Systems.Workshop.WarlordWorkshopSystem.Instance?.AddWorkshop(w.StringId, BanditMilitias.Systems.Workshop.WorkshopType.ArmorSmith);
                    }
                    catch (Exception ex) { DebugLogger.Warning("WarlordCareer", $"Failed to add armorsmith workshop: {ex.Message}"); }
        ...
        {
            if (!IsEnabled) return;
            // GetAllWarlords() IsAlive filtreli — sadece tier filtresi gerekli
