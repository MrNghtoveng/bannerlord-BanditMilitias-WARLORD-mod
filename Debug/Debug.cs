...
namespace BanditMilitias.Debug
{
    ...
    public class DebugPanel
    {
        ...
        public void Update()
        {
            ...
            // Handle keys BEFORE early return so we can enable test mode even when overlay is hidden
            HandleKeyInputs();
            ...
            // L → sadece panel toggle (guard kontrolünden sonra, timer dışında)
            if (Input.IsKeyDown(InputKey.LeftShift) && Input.IsKeyDown(InputKey.LeftAlt)
        ...
        private void HandleKeyInputs()
        {
            if (Input.IsKeyDown(InputKey.LeftShift) && Input.IsKeyDown(InputKey.LeftAlt))
            {
                // BUG 1 & 2 FIX: J ve M guard'dan önce çalışmalı (HandleKeyInputs Update'in başında çağrılıyor)
                if (Input.IsKeyPressed(InputKey.J) || Input.IsKeyPressed(InputKey.M))
                {
                    ToggleTestMode();
                }

                // L → sadece panel toggle (guard altında kalabilir ama kullanıcı guard üstünde de çalışmasını istiyor olabilir)
                // Ancak kullanıcı örneğinde L guard'ın altında.
            }
        }
        ...
        private void ToggleTestMode()
        {
            if (Settings.Instance != null)
            {
                ...
                if (Settings.Instance.TestingMode)
                {
                    ...
                    InformationManager.DisplayMessage(new InformationMessage(
                        $"Current Militias: {count}", Colors.Cyan));
                }
            }
            else
            {
                InformationManager.DisplayMessage(new InformationMessage(
                    "[BanditMilitias] Settings not loaded — MCM kurulu mu?", Colors.Red));
            }
        }
        ...
        [CommandLineFunctionality.CommandLineArgumentFunction("toggle_debug", "bandit")]
        public static string ToggleDebugCommand(List<string> _)
        {
            if (Campaign.Current == null) return "[bandit.toggle_debug] Hata: Aktif kampanya yok.";
            Instance.Toggle();
        ...
        {
            if (Campaign.Current == null) return "[bandit.toggle_test_mode] Hata: Aktif kampanya yok.";
            if (Settings.Instance == null) return "Settings not loaded. Is MCM installed and enabled?";
            Settings.Instance.TestingMode = !Settings.Instance.TestingMode;
            Settings.Instance.ShowTestMessages = Settings.Instance.TestingMode;

            try
            {
                // MCM Save call via reflection to avoid ambiguity with TaleWorlds.Library.Save extensions
                Settings.Instance.GetType().GetMethod("Save")?.Invoke(Settings.Instance, null);
            }
            catch (Exception ex)
            {
                Infrastructure.FileLogger.LogWarning($"MCM SaveSettings failed: {ex.Message}");
            }
        ...
        private void RegisterCommands()
        {
            ...
            _commands["toggle_test"] = _ => ToggleTestMode();
        }
        ...
        {
            if (Campaign.Current == null) return "[bandit.crypto_force_spawn] Hata: Aktif kampanya yok.";
        ...
        {
            if (Campaign.Current == null) return "[bandit.force_spawn] Hata: Aktif kampanya yok.";
            Instance.ForceSpawnCommand(args?.ToArray() ?? Array.Empty<string>());
        ...
        {
            if (Campaign.Current == null) return "[bandit.exec] Hata: Aktif kampanya yok. Lütfen bir oyun yükleyin.";
            if (args == null || args.Count == 0) return "Kullanım: bandit.exec <komut> [argümanlar...]";

            Instance.ExecuteCommand(string.Join(" ", args));
            return "Komut çalıştırıldı.";
        }
        ...
        private void SpawnCommand(string[] args)
        {
            ...
            {
                InformationManager.DisplayMessage(
                    new InformationMessage($"[Debug] Geçersiz sayı: '{args[0]}'. Varsayılan: 1", Colors.Yellow));
                count = 1;
            }
            ...
            {
                InformationManager.DisplayMessage(new InformationMessage("[Debug] Hata: Uygun sığınak (hideout) bulunamadı.", Colors.Red));
                return;
            }
            ...
            if (spawner == null)
            {
                InformationManager.DisplayMessage(new InformationMessage("[Debug] Hata: Spawning sistemi bulunamadı.", Colors.Red));
                return;
            }
